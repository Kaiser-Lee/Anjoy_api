...
//                    .getColumn(gk.getColumn());
        ...
        while (iter.hasNext()) {
            ...
            if(CollectionUtils.isNotEmpty(keyColumns) && keyColumns.size() == 1){
                keyColumnName = MyBatis3FormattingUtilities
                    .getEscapedColumnName(keyColumns.get(0));
            }
            String currentColumnName = MyBatis3FormattingUtilities
                .getEscapedColumnName(introspectedColumn);
            String value;
            if(stringHasValue(keyColumnName) && keyColumnName.equals(currentColumnName)){
                continue;
            }else if("OPER_TIME".equalsIgnoreCase(currentColumnName)){
                value = "now()";
            }else if("VALIDITY".equalsIgnoreCase(currentColumnName)){
                value = "${VALID}";
            }else if("VERSION".equalsIgnoreCase(currentColumnName)){
            	value = "${VERSION}";
            }else{
