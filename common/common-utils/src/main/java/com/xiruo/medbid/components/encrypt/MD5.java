package com.xiruo.medbid.components.encrypt;
...
/**
 * MD5 hash generator.
 * This class takes as input a message of arbitrary length and prod?s
 * as output a 128-bit "fingerprint" or "message digest" of the input.
 * It is conjectured that it is computationally infeasible to prod?
 * two messages having the same message digest, or to prod? any
 * message having a given pre-specified target message digest. The MD5
 * algorithm is intended for digital signature applications, where a
 * large file must be "compressed" in a secure manner before being
 * encrypted with a private (secret) key under a p lic-key cryptosystem
 * s h as RSA.
 * <p>
...
public class MD5 {
   ...
   * @param br Array of bytes to be hashed.
   * @param offset Offset to br array.
   * @param length number of bytes to hash.
   */
  private void update(MD5State state, byte br[], int offset, int length) {

    finalState.valid = false;

    // if length goes beyond the end of the br, cut it short.
    if ((length + offset) > br.length) {
      length = br.length - offset;
    }

    // compute number of bytes mod 64
    // this is what we have sitting in a br
    ...
    int i = 0;
    if (length >= partlen) {
      System.arraycopy(br, offset, state.br, index, partlen);
      transform(state, decode(state.br, 64, 0));
      for (i = partlen; (i + 63) < length; i += 64) {
        transform(state, decode(br, 64, i));
      }
      index = 0;
    }
    ...
    if (i < length) {
      for (int start = i; i < length; i++) {
        state.br[index + i - start] = br[i + offset];
      }
    }
  }
   ...
   * @param length number of bytes to hash.
   */
  public void update(byte br[], int offset, int length) {
    update(workingState, br, offset, length);
  }
   ...
   * @param length number of bytes to hash.
   */
  public void update(byte br[], int length) {
    update(br, 0, length);
  }
   ...
   * @param br Array of bytes to be hashed.
   */
  public void update(byte br[]) {
    update(br, 0, br.length);
  }
   ...
   * the hash is reqsted and it is invalidated when the hash
   * is updated.
   */
  private MD5State finalState = new MD5State();
  /**
   * Temporary br cached here for performance reasons.
   */
  private int[] decodeBr = new int[16];
  /**
  ...
  ,
    ...
    /**
     * 64-byte br (512 bits) for storing to-be-hashed characters
     */
    public byte br[] = new byte[64];
    ...
    public void copy(MD5State from) {
      System.arraycopy(from.br, 0, this.br, 0, this.br.length);
  ...
  private static String toHex(byte hash[]) {
    StringBuffer bf = new StringBuffer(hash.length * 2);
    for (int i = 0; i < hash.length; i++) {
      ...
      bf.append(Integer.toHexString(intVal));
    }
    return bf.toString();
  }
  ...
  private int[] decode(byte br[], int len, int offset) {
    int i, j;
    for (i = j = 0; j < len; i++, j += 4) {
      decodeBr[i] = ((int) (br[j + offset] & 0xff)) |
              (((int) (br[j + 1 + offset] & 0xff)) << 8) |
              (((int) (br[j + 2 + offset] & 0xff)) << 16) |
              (((int) (br[j + 3 + offset] & 0xff)) << 24);
    }
    return decodeBr;
  }
