...
public class SchedulerHelper {
    private static final String CONFIG_FILE="config/quartz-job.properties";
    private static final String IDENTITY_JOB_PREFIX="job_";
    private static final String IDENTITY_TRIGGER_PREFIX="trigger_";
    private static final String SCHEDULER_KEY_JOBSERVICE="job_service";
    ...
    public void updateAndRemoveJob(JobEntity job) {
        try {
            JobDetail jobDetail = scheduler.getJobDetail(getJobKey(job));
            if(jobDetail != null) {
                JobEntity oldEntitiy = (JobEntity) jobDetail.getJobDataMap().get("jobEntity");
                if(job.getJobStatus() == 0) {
                    //已停止的任务从调度器中删除
                    scheduler.deleteJob(getJobKey(job));
                    return;
                }else if (!oldEntitiy.getCronExpr().equals(job.getCronExpr())
                        || !oldEntitiy.getJobObject().equals(job.getJobObject())
                        || !oldEntitiy.getJobMethod().equals(job.getJobMethod())) {
    ...
    public static JobEntityService getJobService(JobExecutionContext context)
    {
        try {
            return (JobEntityService) context.getScheduler().getContext().get(SchedulerHelper.SCHEDULER_KEY_JOBSERVICE);
        } catch (SchedulerException e) {
